<template>
     <div id="desktop">
        <div class="d-flex justify-content-between align-items-center">
            <strong class="me-4 text-secondary" style="width:40px">#{{collecte.id}}</strong>
            <div class="w-100 d-flex flex-column align-items-md-center flex-md-row-reverse justify-content-md-between">
                ...
                <div class="w-100 d-flex align-items-center justify-content-between">
                    ...
                    <div class="mt-2">
                        <div class="text-warning" v-if="!collecte.enqueteur_nom">
                            <span title="Contrôleur non programmé">
                                <user-image name="?" className="me-1" size="sm" />
                            </span>
                        </div>
                        <div v-else class="fw-light fs-7 d-flex align-items-center">
                            <span :title=firstLetterControlerName(collecte.enqueteur_nom)>
                    ...
                    <div v-else-if="collecte.result_var && collecte.result_var == 'null' && !editable" class="badge fs-6 text-uppercase me-md-2" :class="classNameFromSAMI(collecte.result_var)">?</div>
                </div>
            </div>
        </div>
    </div>
...
@media screen and (max-width: 785px) {
  /* styles à appliquer lorsque l'écran est inférieur ou égal à 785px */
  #mobile {
    display: block;
  }
  #desktop {
...
@media screen and (min-width: 786px) {
  /* styles à appliquer lorsque l'écran est supérieur à 785px */
  #mobile {
    display: none;
  }
}
...
export default {
	...
	methods: {
        /**
         * Retourne le nom du controleur avec la premiere lettres de son prénom
         * 
         * @param {string} controleurName 
         * 
         * @returns {string} Nom arrangé
         */
        firstLetterControlerName(controleurName){
            const words = controleurName.split(' ');
            let nameReturn = words[0] + " " + words[1].slice(0,1)
            if (words[2]){
                nameReturn = nameReturn + " " + words[2].slice(0,1)
            }
            return nameReturn + "."
        },
		/**
